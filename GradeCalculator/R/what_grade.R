...
#'
#' This function calculates for its user the percent they need on their final exam to get their desired final grade in a course.
...
#' course grade and returns what percent is needed on the final to achieve that grade. If the percent needed is higher than 100%
...
#' @export
what_grade <- function(assignments_weight, grades_recieved, desired_final_grade) {
  current_grades <- assignments_weight*(grades_recieved/100)
  sum_current_grades <- sum(current_grades)
  final_weight <- 100 - sum(assignments_weight)
  grade_needed = ((desired_final_grade-sum_current_grades)/final_weight) *100
  ifelse(grade_needed > 100, "Oops! Better Luck Next Time :(", paste0(grade_needed, "%"))
}
